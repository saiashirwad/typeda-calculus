...
type List<value extends Token[]> = constructor<
  Token,
  "list",
  { value: value }
>
type Num<value extends number> = constructor<
  Token,
  "number",
  { value: value }
>
type Sym<value extends string> = constructor<
  Token,
  "symbol",
  { value: value }
>
type Str<value extends string> = constructor<
  Token,
  "string",
  { value: value }
>
...
type shiftNumber<s extends State<any>> =
  s["unscanned"] extends (
    numPattern<infer digit, infer rest>
  ) ?
    State<
      trimLeft<rest>,
...
type isDelimiter<char extends string> =
...
type isSymbol<T extends Token, V extends string> =
  T extends (
    {
      type: "symbol"
      value: V
...
type transformDefinition<T extends Token> =
  T extends (
    {
      type: "list"
      value: List3<
...
type extractBindings<T extends Token[]> =
  T extends (
    [
      List<
        List2<
...
type transformLet<t extends Token> =
  t extends (
    {
      type: "list"
      value: [
        ...
        {
          type: "list"
          value: infer bindings extends Token[]
        },
...
type popStack<s extends State<any>> =
  s["stack"] extends (
    [
...
type parse<s extends State<any>> =
  s["unscanned"] extends "" ?
...
type matchList<T, first extends T, rest extends T[]> = [
  first,
  ...rest
]

type eval<tok extends Token, env extends Environment> =
  tok extends Num<any> ? tok
  : tok extends Sym<infer sym> ? lookupEnv<env, sym>
  : tok extends (
    List<matchList<Token, infer first, infer rest>>
  ) ?
...
type result2 = eval<
  parse<initialState<"(sub 6 (add 3 2))">>[0],
  initialEnv
>
